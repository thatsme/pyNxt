...
class GetSellOffers(Parent):
    def __init__(self, currency=None, account=None, availableOny=False, sortByRate=False, ri=None,  rb=None ):
        """
            ...
            GetSellOffers take a default 1/2 parameter as explained in NXT API Documentation

            API is working with GET method

            https://nxtwiki.org/wiki/The_Nxt_API#Get_Sell_Offers
        ...
        super(GetSellOffers, self).__init__(rt="getSellOffers", data=self.data, ri=self.ri, rb=self.rb)
    ...
    def run(self):
        super(GetSellOffers, self).run()                                         # calls 'BaseGet.run()'
    ...
    def getData(self, key=None):
        ...
        """
        return super(GetSellOffers, self).getData(key)                           # calls 'BaseGet.getData()'
