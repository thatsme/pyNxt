...
final class Curve25519 {
    ...
    public static void keygen(byte[] P, byte[] s, byte[] k) {
        ...
        System.out.println("ORDER --- >>>>>>>>>>>>>>"+toHexString(ORDER));
        System.out.println("");

        System.out.println("");
        System.out.println("Curve keygen secret key before clamp "+toHexString(k));
        clamp(k);
        System.out.println("Curve keygen secret key after clamp  "+toHexString(k));
        System.out.println("");
        core(P, s, k, null);
        System.out.println("Curve keygen after core  "+toHexString(k));
        System.out.println("Curve Keygen public key  "+toHexString(P));
        System.out.println("");
    }
    ...
    private static final class long10 {
        ...
        public void p () {
            System.out.println("long10 ->"+this._0+" "+this._1+" "+this._2+" "+this._3+" "+this._4+" "+this._5+" "+this._6+" "+this._7+" "+this._8+" "+this._9);
        }
    }
    ...
    private static void divmod(byte[] q, byte[] r, int n, byte[] d, int t) {
        ...
        while (n-- >= t) {
            ...
            System.out.println("divmod float --> "+((rn << 16) | ((r[n] & 0xFF) << 8)));
            System.out.println("divmod int --> "+z);
            if (n>0) {
                z |= (r[n-1] & 0xFF);
            }
            z/=dt;
    ...
    private static void pack(long10 x,byte[] m) {
        ...
        // System.out.println("pack function t value -->"+strLong);
        // System.out.println("pack function t value (byte)--> "+(byte)t+" "+(long)t);
        m[ 0] = (byte)t;

        strLong = Long.toString((t >>8));
        // System.out.println("pack function t value -->"+strLong);
        // System.out.println("pack function t value (byte)-->"+(byte)(t >>8)+" "+(long)(t >>8));
        m[ 1] = (byte)(t >> 8);
        strLong = Long.toString((t >>16));
        // System.out.println("pack function t value -->"+strLong);
        // System.out.println("pack function t value (byte)-->"+(byte)(t >>16)+" "+(long)(t >>16));
        m[ 2] = (byte)(t >> 16);

        strLong = Long.toString((t >>24));
        // System.out.println("pack function t value -->"+strLong);
        // System.out.println("pack function t value (byte)-->"+(byte)(t >>24)+" "+(long)(t >>24));
        m[ 3] = (byte)(t >> 24);
        t = (t >> 32) + (x._2 << 19);

        strLong = Long.toString(t);
        // System.out.println("pack function t value -->"+strLong);
        // System.out.println("pack function t value (byte)--> "+(byte)t+" "+(long)t);
        m[ 4] = (byte)t;
        ...
        strLong = Long.toString((t >>8));
        // System.out.println("pack function t value -->"+strLong);
        // System.out.println("pack function t value (byte)-->"+(byte)(t >>8));
        m[ 5] = (byte)(t >> 8);

        strLong = Long.toString((t >>16));
        // System.out.println("pack function t value -->"+strLong);
        // System.out.println("pack function t value (byte)-->"+(byte)(t >>16));
        m[ 6] = (byte)(t >> 16);

        strLong = Long.toString((t >>24));
        // System.out.println("pack function t value -->"+strLong);
        // System.out.println("pack function t value (byte)-->"+(byte)(t >>24));
        m[ 7] = (byte)(t >> 24);
    ...
    private static long10 mul(long10 xy, long10 x, long10 y) {
        ...
        xy._8 = (t & ((1 << 26) - 1));
        xy._9 += (t >> 26);
        // System.out.println("Output of mul --> bytes"+xy._0+" "+xy._1+" "+xy._2+" "+xy._3+" "+xy._4+" "+xy._5+" "+xy._6+" "+xy._7+" "+xy._8+" "+xy._9);
        return xy;
    }
    ...
    private static void core(byte[] Px, byte[] s, byte[] k, byte[] Gx) {
        ...
        for (i = 32; i--!=0; ) {
            System.out.println("ma cazzo i "+i);
            if (i==0) {
                i=0;
            }
            for (j = 8; j--!=0; ) {
                ...
                // System.out.println("bit1 "+bit1+" "+k[i]+" "+j+" "+(k[i] & 0xFF)+" "+((k[i] & 0xFF) >> j));
                int bit0 = ~(k[i] & 0xFF) >> j & 1;
                System.out.println("bit0 "+bit0+" "+k[i]+" "+~(k[i] & 0xFF));
                long10 ax = x[bit0];
                long10 az = z[bit0];
                long10 bx = x[bit1];
                long10 bz = z[bit1];
        ...
        if (s!=null) {
            ...
            cpy32(temp1, ORDER);
            System.out.println("CORE S  --> "+s);
            System.out.println("CORE temp2  --> "+temp2);
            System.out.println("CORE temp3  --> "+temp3);
            System.out.println("CORE temp1  --> "+temp1);
            cpy32(s, egcd32(temp2, temp3, s, temp1));
            if ((s[31] & 0x80)!=0)
                mula_small(s, s, 0, ORDER, 32, 1);
        } else {
            System.out.println("CORE S  --> IS NULL !!!! ");
        }
    }
