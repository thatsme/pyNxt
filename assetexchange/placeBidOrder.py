...
class PlaceBidOrder(Parent):
    def __init__(self, asset = None, quantityQNT=0, priceNQT=0, secretPhrase=None,  publicKey = None, feeNQT = None, deadline = 0, referencedTransactionFullHash = None, broadcast=False, phasing = None, message=None, recipientPublicKey=None, rec=None ):
        """
            ...
            PlaceBidOrder take a default 5 parameter as explained in NXT API Documentation
            ...
            https://nxtwiki.org/wiki/The_Nxt_API#Place_Bid_Order
        ...
        super(PlaceBidOrder, self).__init__(rt="placeBidOrder", data=self.data, phasing=self.phasing, message=self.message, rec=self.rec)

    def run(self):
        super(PlaceBidOrder, self).run()                # calls 'BasePost.run()'

    def getData(self, key=None):
        return super(PlaceBidOrder, self).getData(key)  # calls 'BasePost.getData()'
